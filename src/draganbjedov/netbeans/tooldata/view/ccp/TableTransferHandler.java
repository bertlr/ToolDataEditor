...
public class TableTransferHandler extends TransferHandler {
	...
	public boolean importData(TransferHandler.TransferSupport support) {
		...
		try {
			rows = (List<Pair<Integer, ArrayList<String>>>) support.getTransferable().getTransferData(TableRowTransferable.TOOLDATA_ROWS_DATA_FLAVOR);
		...
		TOOLDATATableModel model = (TOOLDATATableModel) table.getModel();
	...
	/**
	 * We only support importing TOOLDATA_ROWS_DATA_FLAVOR.
	 */
	@Override
	public boolean canImport(TransferHandler.TransferSupport support) {
		final boolean dataFlavorSupported = support.isDataFlavorSupported(TableRowTransferable.TOOLDATA_ROWS_DATA_FLAVOR);
		if (!dataFlavorSupported)
			...
			if (dl.isInsertRow()) {
				...
				try {
					List<Pair<Integer, ArrayList<String>>> rows = (List<Pair<Integer, ArrayList<String>>>) support.getTransferable().getTransferData(TableRowTransferable.TOOLDATA_ROWS_DATA_FLAVOR);
					return rows.size() == 1;
				} catch (UnsupportedFlavorException ufe) {
					return false;
				} catch (IOException ioe) {
